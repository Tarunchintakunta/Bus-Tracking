...
const Driver = () => {
  ...
  const handleSave = async () => {
    ...
    if (!driverId || !busRouteNumber) {
      alert('Please enter Driver ID and Bus Route Number.');
      return;
    }
    const collectionRef = collection(firestore, 'driver');

    const q1 = query(collectionRef, where("DriverId", "==", driverId));
    const q2 = query(collectionRef, where("DriverRoute", "==", busRouteNumber));
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.inputContainer}>
        <Icon name="ios-person" size={24} color="#041E42" style={styles.icon} />
        <TextInput
          style={styles.input}
      ...
      <View style={styles.inputContainer}>
        <Icon name="ios-bus" size={24} color="#041E42" style={styles.icon} />
        <TextInput
          style={styles.input}
          placeholder="Enter Bus Route Number"
          value={busRouteNumber}
          onChangeText={(text) => setBusRouteNumber(text)}
        />
      </View>
